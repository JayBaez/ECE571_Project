...
| Graph-based label propagation (min-cut / soft-cut / kNN graph) | Week12 | SSL classifier via similarity graph | P4 | Taught SSL option, conceptually closest to kNN |
...
| Pseudo-labeling / self-training | Simple SSL approach | P4 | Not one of the three SSL methods actually taught (Transductive SVM, Co-training, graph-based) — simpler to implement and explain than those three, but you should decide (per `TEACHER_EXPECTATIONS.md`) whether the rubric wants a course-taught method specifically |
...
- **Problem 4 (semi-supervised):** Supervised baseline (any P1 classifier) → one of Transductive SVM / Co-training / graph-based (taught) — or pseudo-labeling as a simpler, not-explicitly-taught alternative, pending your decision.
